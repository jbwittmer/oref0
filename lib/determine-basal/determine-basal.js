    ...
    */
    ...
    if (basaliob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && profile.current_basal >= 0.3 && bg > threshold) {
            ...
            // (or enough insulin to cover half of COB, or maxBolus, whichever is smallest)
            mealInsulinReq = meal_data.mealCOB / profile.carb_ratio;
            if ( iob_data.iob < mealInsulinReq / 2 ) {
                halfMealInsulinReq = round(mealInsulinReq/2-iob_data.iob,1);
                console.error("IOB",iob_data.iob,"<< COB",meal_data.mealCOB+"; insulinReq/2 =",insulinReq/2+"; halfMealInsulinReq =",halfMealInsulinReq);
                microBolus = round(Math.min(insulinReq/2,maxBolus,halfMealInsulinReq),1);
            }
